...
// Private method, for use inside the class only
...
- (id)addNewApp:(NSString *)appName withTheme:(NSString *)theme sequence:(int)sequence inWeek:(int)week
{
    // Create a new App object
    NSManagedObject *a = [self addNew:@"App"];
    // Configure it
    [a setValue:appName forKey:@"appName"];
    [a setValue:theme forKey:@"theme"];
    [a setValue:[NSNumber numberWithInt:sequence] forKey:@"sequence"];
    [a setValue:[NSNumber numberWithInt:week] forKey:@"week"];
    
    return a;
}

- (id)addNewTopic:(NSString *)topicName withDescription:(NSString *)topicDescription andNumber:(int)number
{
    // Configure a new Topic object
    NSManagedObject *t = [self addNew:@"Topic"];
    // Configure it
    [t setValue:topicName forKey:@"topicName"];
    [t setValue:topicDescription forKey:@"topicDescription"];
    [t setValue:[NSNumber numberWithInt:number] forKey:@"topicNumber"];
