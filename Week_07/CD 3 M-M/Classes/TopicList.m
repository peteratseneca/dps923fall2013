...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSManagedObject *o = [self.model.frc_topic objectAtIndexPath:indexPath];
    
    cell.textLabel.text = [o valueForKey:@"topicName"];
    cell.detailTextLabel.text = [o valueForKey:@"topicDescription"];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([segue.identifier isEqualToString:@"toTopicView"]) {
        ...
        NSManagedObject *o = [self.model.frc_topic objectAtIndexPath:[self.tableView indexPathForSelectedRow]];
        
        //Configure the next VC
        TopicView *nextVC = (TopicView *)segue.destinationViewController;
        nextVC.title = [o valueForKey:@"topicName"];
