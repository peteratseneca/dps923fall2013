...
#pragma mark - Web service Building entity
...
- (NSArray *)buildings
{
    ...
    [[Client500 sharedClient] getPath:@"buildings"
                           parameters:nil
                              success:^(AFHTTPRequestOperation *operation, id response) {

                                  // Cast the response
                                  _buildings = (NSArray *)[response objectForKey:@"Collection"];
                                  
                                  // Reference the app's network activity indicator in the status bar
                                  [[UIApplication sharedApplication] setNetworkActivityIndicatorVisible:NO];
                                  
                                  // Then, post a notification, so that a controller object can request the results
                                  [[NSNotificationCenter defaultCenter] postNotificationName:@"SuccessGetBuildings" object:nil];
...
- (NSArray *)spaces
{
    ...
    [[Client500 sharedClient] getPath:@"spaces"
                           parameters:nil
                              success:^(AFHTTPRequestOperation *operation, id response) {
                                  
                                  // Cast the response
                                  _spaces = (NSArray *)[response objectForKey:@"Collection"];

                                  // Reference the app's network activity indicator in the status bar
                                  [[UIApplication sharedApplication] setNetworkActivityIndicatorVisible:NO];
                                  
                                  // Then, post a notification, so that a controller object can request the results
                                  [[NSNotificationCenter defaultCenter] postNotificationName:@"SuccessGetSpaces" object:nil];
